...
class Relation(LightningModule):
    ...
    def train_collate_batch(self, batch):
        ...
        outputs = self.tokenizer(sentences, truncation=True, max_length=512, padding=True,
    ...
    def train_dataloader(self, train_batch_size=8):
        return DataLoader(
    ...
    def val_dataloader(self, valid_batch_size=64):
        return DataLoader(
...
if __name__ == '__main__':
    ...
    testB_ner_path = 'data/testB_ner.txt'  # 上一步得到的ner的结果
    testB_path = 'data_relation/testB.json'  # testB关系数据
    submit_path = 'data_relation/submit_B.txt'  # 生成testB结果
    test_data_processing(tokenizer_fast, testB_ner_path, testB_path)  # 生成测试集

    with open(testB_path, 'r', encoding='utf-8') as f:
