...
class GlobalPointer(LightningModule):
    ...
    def train_dataloader(self, train_batch_size=32):
        return DataLoader(
    ...
    def val_dataloader(self, valid_batch_size=128):
        return DataLoader(
...
if __name__ == '__main__':
    ...
    testB_raw_path = 'data/testB.conll_sent.conll'  # testB数据集
    testB_path = 'data/testB_ner.txt'  # 生成testB数据ner部分

    with open(testB_raw_path, 'r', encoding='utf-8') as f:
        testB_sentences = f.readlines()
    with open(testB_path, 'w', encoding='utf-8') as f:
